    ...
    if (responseText !== '') {
        ...
        if (!isResult) {
            ...
            try {
                ...
                const apiResponse = await fetch('http://localhost:3000/chatgpt/question', {
                    method: 'POST',
                    headers: {
                ...
                currentSecondaryQuestion = result.result; // Store the secondary question
            ...
            try {
                ...
                const apiResponse = await fetch('http://localhost:3000/chatgpt/combine', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`, // Authorization 헤더에 토큰 추가
                ...
                resultMessage2.innerHTML = `<span>${result.result}</span>`;
                ...
                await fetch('http://localhost:3000/post/', {
                    ...
                    body: JSON.stringify({ finalAnswer: result.result, bigQuestionId:mainQuestionId, smallQuestionId:subQuestionId })
                });
